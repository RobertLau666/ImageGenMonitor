        ...
        for i, group in enumerate(groups):
            ...
            for log in group:
                ...
                if npc_name_list_match:
                    ...
                    npc_names = npc_name_list_match.group(2) # ", ".join(eval(npc_name_list_match.group(2)))
...
def generate_bar_chart(record_dict_new):
    ...
    NPC_nums_df = pd.DataFrame({"x": date_list, "y": NPC_nums_list})
    image_nums_df = pd.DataFrame({"x": date_list, "y": image_nums_list})
    ...
    return NPC_nums_df, image_nums_df, NPC_nums_txt, image_nums_txt
...
def pipeline():
    ...
    # 2. 根据json画图
    NPC_nums_df, image_nums_df, NPC_nums_txt, image_nums_txt = generate_bar_chart(record_dict_new)
...
def show_plot():
    with gr.Blocks() as show_plot_blocks:
        with gr.Column():
            ...
            NPC_nums_txt = gr.Textbox('', label="all_NPC_nums", interactive=False, lines=3)
            image_nums_plot = gr.LinePlot(
                x="x",
                y="y",
                overlay_point=True,
                tooltip=["x", "y"],
            ...
            image_nums_txt = gr.Textbox('', label="all_image_nums", interactive=False, lines=3)
            show_ = gr.Button("Show", visible=True)
    show_.click(
        fn=pipeline,
        inputs=[],
        outputs=[NPC_nums_plot, image_nums_plot, NPC_nums_txt, image_nums_txt]
    )
    return show_
...
with gr.Blocks() as demo:
    gr.Markdown("""
                ...
                <strong><em>view offline gen image nums</em></strong>
                </div>
                """)
...
if __name__ == "__main__":
    demo.queue().launch(server_port=8000, server_name="0.0.0.0", share=True)
